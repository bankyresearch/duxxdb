//! # duxx-storage
//!
//! Two layers of storage abstraction:
//!
//! 1. [`Table`] — schema-aware row-oriented placeholder for the future
//!    Lance / Arrow columnar backend. See [PHASE_2_3_PLAN.md] for the
//!    integration plan.
//! 2. [`Storage`] — minimal byte-keyed key/value store. This is what
//!    `MemoryStore` uses for durable `by_id` storage. Pure Rust impls:
//!    [`MemoryStorage`] (in-memory) and [`RedbStorage`] (durable via
//!    [redb]).
//!
